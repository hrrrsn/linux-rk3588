#!/bin/bash
  ...
  debootstrap --arch=arm64 $debian_version $root_dir
  ...
  chroot $root_dir /bin/bash -c "apt-get update; dpkg -i /install/*.deb; \
    apt-get install -y grub-efi-arm64 systemd gpg openssh-client nano curl wget initramfs-tools"

  # Install grub
  mkdir -p $root_dir/boot/efi/EFI/BOOT
  chroot $root_dir /bin/bash -c "grub-install --target=arm64-efi --efi-directory=/boot/efi --bootloader-id=debian --recheck; update-initramfs -u -k all"
  patch $root_dir/etc/grub.d/10_linux < $work_dir/../../patch/10_linux.patch 
  chroot $root_dir /bin/bash -c "update-grub; cp /boot/efi/grub/grubaa64.efi /boot/efi/EFI/BOOT/BOOTAA64.EFI"
  echo "\EFI\debian\grubaa64.efi" > $root_dir/boot/efi/startup.nsh

  uuid=$(blkid -s UUID -o value /dev/loop0p2)
  sed -i "s|root=/dev/loop0p2|root=UUID=$uuid|g" $work_dir/root/boot/grub/grub.cfg
  echo "UUID=$uuid /               ext4    errors=remount-ro 0       1" >> $root_dir/etc/fstab

  # Install desktop environment
  if [ ! -z "$install_desktop" ]; then
    chroot $root_dir /bin/bash -c "apt-get install -y $install_desktop"
  fi
  
  # Unmount and detach
  rm -rf $root_dir/install
  chroot $root_dir /bin/bash -c "apt-get clean -y; sync; fstrim -a"
